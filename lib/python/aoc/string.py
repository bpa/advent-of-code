...
def comb(input, r, map=None, one_line=False):
    import re

    compiled = re.compile(r)
    if one_line:
        if map:
            return [map(i) for i in compiled.findall(input)]
        else:
    ...
    def generate():
        if map:
            for line in input.splitlines():
                yield [map(i) for i in compiled.findall(line)]
        else:
            for line in input.splitlines():
                yield compiled.findall(line)
...
def regex(input, r, map_each=None, map=None, one_line=False):
    ...
    def match(compiled, line, line_no):
        ...
        if result:
            if map_each:
                args = [map_each(v) for v in result.groups()]
            else:
                args = result.groups()
            if map:
                return map(*args)
